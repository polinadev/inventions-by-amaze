...
- A single transparent Inventions wordmark serves the hero and compact headers at its intrinsic aspect ratio. Its asymmetric transparent safe area puts the upper `I` medallion on the true visual axis while preserving the complete lightning `I`; the official A\Maze signature is composed separately for `BY` / `PAR` localization without CSS cropping or blend modes.
