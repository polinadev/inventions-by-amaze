...
const BOOKING_URL = "https://offthecouch.io/book/AmazeOldPort";
const MAPS_URL =
...
const cocktails: CocktailItem[] = [
  { id: "science-experiment", number: "01", name: "The Science Experiment", price: "$16", description: "Mezcal · Aperol · lime · soda · absinthe", service: "A tall, aromatic spritz with a restrained absinthe finish.", image: sitePath("images/cocktails/science-experiment.png") },
  { id: "oscillator", number: "02", name: "Oscillator", price: "$16", description: "Canadian rye · chocolate liqueur · butterscotch · Chartreuse · chocolate bitters · chili", service: "Spirit-forward, compact and deliberate with a warm final charge.", image: sitePath("images/cocktails/oscillator.png") },
  { id: "teslas-carre", number: "03", name: "Tesla’s Carré", price: "$17", description: "Rye · Grand Marnier · sweet vermouth · Bénédictine · smoked dome", service: "Presented beneath a smoked dome and revealed at the table.", image: sitePath("images/cocktails/teslas-carre.png") },
  { id: "radioactive-sour", number: "04", name: "Radioactive Sour", price: "$16", description: "White rum · Midori · lemon · simple syrup · foamer", service: "A tart, luminous sour with a soft foamed cap.", image: sitePath("images/cocktails/radioactive-sour.png") },
  { id: "wardenclyff", number: "05", name: "Wardenclyff", price: "$16", description: "Calvados · Amaro Montenegro or Nonino · red vermouth · Angostura · smoke bubble", service: "The bubble seals the rim and is broken at the table to release the aroma.", image: sitePath("images/cocktails/wardenclyff.png") },
  { id: "alternating-current", number: "06", name: "Alternating Current", price: "$15", description: "Tanqueray · Grand Marnier · cardamom bitters · sparkling water · lemon · dried orange · butterfly-pea tea", service: "A long electric highball with layered citrus and cardamom.", image: sitePath("images/cocktails/alternating-current.png") },
  { id: "fire-conductor", number: "07", name: "Fire Conductor", price: "$15", description: "Fireball or Crown Royal Apple · lime · maple syrup · ginger beer", service: "Cold copper, warming spice and a sharp ginger finish.", image: sitePath("images/cocktails/fire-conductor.png") },
  { id: "seasonal", number: "08", name: "Cocktail of the Season", price: "$17", description: "Ask your bartender for the current invention", label: "Seasonal", service: "A rotating invention built around what is currently in season.", image: sitePath("images/cocktails/seasonal.png") },
];
...
const bites: MenuItem[] = [
  ...
  { name: "House-Roasted Nuts", price: "$6", description: "Caramel · spice" },
  { name: "Olives or Pickles", price: "$5.50" },
];
...
const frText: Record<string, string> = {
  ...
  "A tall, aromatic spritz with a restrained absinthe finish.": "Un spritz allongé et aromatique, avec une finale d’absinthe tout en retenue.",
  "Spirit-forward, compact and deliberate with a warm final charge.": "Corsé, compact et précis, avec une finale chaleureuse.",
  "Presented beneath a smoked dome and revealed at the table.": "Présenté sous cloche fumée et dévoilé à table.",
  "A tart, luminous sour with a soft foamed cap.": "Un sour acidulé et lumineux, coiffé d’une mousse soyeuse.",
  "The bubble seals the rim and is broken at the table to release the aroma.": "La bulle scelle le bord du verre, puis éclate à table pour libérer ses arômes.",
  "A long electric highball with layered citrus and cardamom.": "Un highball électrique et allongé, aux notes superposées d’agrumes et de cardamome.",
  "Cold copper, warming spice and a sharp ginger finish.": "Le froid du cuivre, la chaleur des épices et une finale franche de gingembre.",
  "A rotating invention built around what is currently in season.": "Une création renouvelée au fil des saisons.",
  ...
  "Cocktails, curious plates and immersive escape games in Montréal’s Old Port.": "Cocktails, bouchées inventives et jeux d’évasion immersifs au Vieux-Port de Montréal.",
  ...
  "Select a cocktail to reveal its illustrated serving card.": "Sélectionnez un cocktail pour découvrir sa carte illustrée.",
  "Cocktails": "Cocktails", "Zero Proof": "Sans alcool", "Beer & Wine": "Bières et vins", "Small Bites": "Petites bouchées",
  "Select a drink · view the illustration": "Sélectionnez un cocktail · découvrez l’illustration",
  ...
  "Private events · Team building · 5 à 7": "Événements privés · Team building · 5 à 7", "Bring your": "Réunissez vos", "brightest minds.": "esprits les plus brillants.",
  ...
  "Plan a group event": "Planifier un événement de groupe", "Find the": "Suivez le", "current.": "courant.", "Opening hours": "Heures d’ouverture",
...
const localizeItem = <T extends MenuItem>(item: T, locale: Locale): T => ({
  ...
  ...(Object.prototype.hasOwnProperty.call(item, "service") ? { service: translate(locale, (item as CocktailItem).service) } : {}),
});
...
function CocktailDetail({ cocktail, id, className = "", onClose, locale }: { cocktail: CocktailItem; id: string; className?: string; onClose: () => void; locale: Locale }) {
  ...
  return (
    <section className={`cocktail-detail ${className}`} id={id} aria-labelledby={`${id}-title`}>
      ...
      <div className="cocktail-detail-topline"><span>{isFr ? "Dossier du cocktail · no" : "Cocktail dossier · No."} {cocktail.number}</span><button type="button" onClick={onClose} aria-label={isFr ? `Fermer les détails de ${cocktail.name}` : `Close ${cocktail.name} details`}>{isFr ? "Fermer" : "Close"} ×</button></div>
      ...
      <div className="cocktail-art"><img src={cocktail.image} alt={isFr ? `Illustration gravée de ${cocktail.name}` : `Engraved illustration of ${cocktail.name}`} /></div>
      <p className="cocktail-service">{cocktail.service}</p>
          ...
          return (
            <div className={`cocktail-entry${isActive ? " is-active" : ""}`} key={cocktail.id}>
              <button className="cocktail-row" type="button" ref={(node) => registerButton(cocktail.id, node)} onClick={() => onSelect(cocktail.id)} aria-expanded={isActive} aria-controls={`${desktopPanelId} ${mobilePanelId}`}>
                <span className="cocktail-glass-icon" aria-hidden="true"><img src={cocktail.image} alt="" /></span>
                <span className="cocktail-row-copy">
                  ...
                  <span className="cocktail-row-cue">{isActive ? (isFr ? "Sélectionné · fermer les détails" : "Selected · close details") : (isFr ? "Voir l’illustration" : "View cocktail illustration")}</span>
                </span>
    ...
    <main id="top" lang={locale}>
      <header className="site-header">
        <a className="brand" href="#top" aria-label={isFr ? "Accueil — Inventions par A\\Maze" : "Inventions by A Maze home"} onClick={closeMenu}>
          <img className="brand-lockup" src={sitePath("images/inventions-header-lockup.png")} alt="" />
        </a>
        <nav className="desktop-nav" aria-label={isFr ? "Navigation principale" : "Primary navigation"}>
          <a href="#bar">{t("The bar")}</a><a href="#menu">Menu</a><a href="#games">{t("Escape games")}</a><a href="#groups">{t("Groups")}</a><a href="#visit">{t("Visit")}</a>
        </nav>
        <div className="header-actions">
          <a className="language-link" href={sitePath(isFr ? "" : "fr/")} hrefLang={isFr ? "en" : "fr"} lang={isFr ? "en" : "fr"} aria-label={isFr ? "English version" : "Version française"}>{isFr ? "EN" : "FR"}</a>
          <a className="button button-small header-book" href={BOOKING_URL} target="_blank" rel="noreferrer">{t("Book an experience")}</a>
        ...
        <nav className={`mobile-nav${mobileOpen ? " is-open" : ""}`} id="mobile-navigation" aria-label={isFr ? "Navigation mobile" : "Mobile navigation"}>
          <a href="#bar" onClick={closeMenu}>{t("The bar")}</a><a href="#menu" onClick={closeMenu}>Menu</a><a href="#games" onClick={closeMenu}>{t("Escape games")}</a><a href="#groups" onClick={closeMenu}>{t("Groups")}</a><a href="#visit" onClick={closeMenu}>{t("Visit")}</a><a href={BOOKING_URL} target="_blank" rel="noreferrer">{t("Book an experience")} ↗</a>
        </nav>
      </header>
      ...
      <section className="hero" aria-labelledby="hero-title">
        <div className="hero-copy">
          ...
          <h1 id="hero-title"><span className="sr-only">{isFr ? "Inventions par A\\Maze. Le courant passe." : "Inventions by A Maze. The current is on."}</span><img className="hero-lockup" src={sitePath("images/inventions-wordmark-lockup.png")} alt="" /></h1>
          <p className="hero-lede">{t("Cocktails, curious plates and immersive escape games in Montréal’s Old Port.")}</p>
          <div className="hero-actions">
            <a className="button button-solid" href={BOOKING_URL} target="_blank" rel="noreferrer">{t("Book an experience")}</a>
        ...
        <div className="hero-visual">
          <img className="hero-bar-photo" src={sitePath("images/bartender.jpg")} alt={isFr ? "Un barman d’Inventions prépare un cocktail au bar du Vieux-Port" : "An Inventions bartender preparing a cocktail at the Old Port bar"} />
          <figure className="hero-cocktail-inset"><img src={sitePath("images/venue-cocktails-current.jpg")} alt={isFr ? "Trois cocktails signatures servis chez Inventions" : "Three signature cocktails served at Inventions"} /><figcaption>{isFr ? "Cocktails maison · Vieux-Port" : "Original cocktails · Old Port"}</figcaption></figure>
        </div>
      </section>
      ...
      <section className="bar-section section-shell" id="bar">
        <div className="bar-gallery">
          <figure className="bar-photo bar-photo-main"><img src={sitePath("images/venue-overview-current.jpg")} alt={isFr ? "Le véritable salon d’Inventions dans le Vieux-Port de Montréal" : "The real Inventions lounge in Montréal’s Old Port"} loading="lazy" /><figcaption>Inventions · {isFr ? "480, rue Saint-Jean" : "480 Rue Saint-Jean"}</figcaption></figure>
          <figure className="bar-photo bar-photo-detail"><img src={sitePath("images/venue-drink-current.jpg")} alt={isFr ? "Un cocktail signature servi dans une tasse en cuivre chez Inventions" : "A signature cocktail served in a copper mug at Inventions"} loading="lazy" /></figure>
        </div>
        <div className="bar-copy">
          <p className="eyebrow">{t("The bar · After the laboratory")}</p>
          <h2>{t("An evening")}<br />{t("in progress.")}</h2>
          <p className="section-lede">{t("Come early or stay after your game for original cocktails, local craft beer, wine and a tight menu of small bites. Twenty-five seats, low light, zero pressure to solve anything before the first sip.")}</p>
      ...
      <section className="menu-section" id="menu" aria-labelledby="menu-title">
        <div className="menu-heading section-shell">
          ...
          <p>{t("Select a cocktail to reveal its illustrated serving card.")}</p>
        </div>
        ...
        <div className={`menu-board section-shell${menuTab === "cocktails" ? " menu-board-cocktails" : ""}`} id="menu-panel">
          <div className="menu-content">
            {menuTab === "cocktails" && <><div className="menu-content-heading"><h3>{t("Signature cocktails")}</h3><span>{t("Select a drink · view the illustration")}</span></div><CocktailMenu items={localizedCocktails} activeId={activeCocktailId} onSelect={(id) => setActiveCocktailId((current) => current === id ? null : id)} onClose={closeCocktail} registerButton={(id, node) => { cocktailButtonRefs.current[id] = node; }} locale={locale} /></>}
      ...
      <footer className="site-footer">
        <div className="footer-brand"><Monogram /><div><strong>INVENTIONS</strong><span>BY A\MAZE</span></div></div>
        <p>{t("Cocktails · Wine · Escape Games")}<br />{t("Montréal Old Port")}</p>
        <div className="footer-links"><a href="https://www.instagram.com/amazemontreal/" target="_blank" rel="noreferrer">Instagram ↗</a><a href="https://www.facebook.com/amazemontreal" target="_blank" rel="noreferrer">Facebook ↗</a><a href={isFr ? "https://www.amazemontreal.com/fr/cartes-cadeaux-jeux-devasion" : "https://www.amazemontreal.com/gift-certificates-escape-room"} target="_blank" rel="noreferrer">{t("Gift cards")} ↗</a></div>
